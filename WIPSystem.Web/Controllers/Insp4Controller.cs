...
{
    public class Insp4Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<Insp4Controller> _logger;

        public Insp4Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<Insp4Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Insp.4' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT Insp4Id, PartNo, LotNo, OutputQty, Status 
            FROM Insp4 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<Insp4ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new Insp4ViewModel
                        {
                            Insp4Id = reader.GetInt32(reader.GetOrdinal("Insp4Id")),
                            ...
                            Insp4ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<Insp4ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new Insp4ViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Insp.4"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Insp4ViewModel model)
        {
        ...
        private void PrepareViewModel(Insp4ViewModel model)
        {
        ...
        public async Task<IActionResult> Insp4Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            Insp4ViewModel viewModel = null;
            ...
            WHERE i.Insp4Id = @Insp4Id";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@Insp4Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new Insp4ViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<Insp4ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_Insp4DetailsModal", viewModel);
        }
        ...
        {
            Insp4ViewModel viewModel = new Insp4ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.Insp4Id = reader.GetInt32(reader.GetOrdinal("Insp4Id"));
                            ...
                            if (Enum.TryParse<Insp4ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = Insp4ProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(Insp4ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(Insp4ViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var agCommand = new SqlCommand(agUpdateCommandText, connection, transaction))
                    {
                        ...
                        agCommand.Parameters.AddWithValue("@Insp4Id", model.Insp4Id);
        ...
        public async Task<IActionResult> Delete(int Insp4Id)
        {
            ...
            string commandText = "DELETE FROM Insp4 WHERE Insp4Id = @Insp4Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@Insp4Id", Insp4Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted Insp4 with ID {Insp4Id}.");
