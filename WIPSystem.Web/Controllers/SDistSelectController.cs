...
{
    public class SDistSelectController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<SDistSelectController> _logger;

        public SDistSelectController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<SDistSelectController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'S-Dist Select' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT SDistSelectId, PartNo, LotNo, OutputQty, Status 
            FROM SDistSelect 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<SDistSelectViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new SDistSelectViewModel
                        {
                            SDistSelectId = reader.GetInt32(reader.GetOrdinal("SDistSelectId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             SDistProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<SDistProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new SDistSelectViewModel   
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "S-Dist Select"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(SDistSelectViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the Sinter table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a Sinter record.");
        ...
        private void PrepareViewModel(SDistSelectViewModel model)
        {
        ...
        public async Task<IActionResult> SDistSelectDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            SDistSelectViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE s.SDistSelectId = @SDistSelectId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@SDistSelectId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new SDistSelectViewModel
                        {
                            SDistSelectId = reader.GetInt32(reader.GetOrdinal("SDistSelectId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<SDistProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_SDistSelectDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            SDistSelectViewModel viewModel = new SDistSelectViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.SDistSelectId = reader.GetInt32(reader.GetOrdinal("SDistSelectId"));
                            ...
                            if (Enum.TryParse<SDistProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = SDistProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(SDistSelectViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(SDistSelectViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE SDistSelectId = @SDistSelectId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sdistCommand = new SqlCommand(SDistUpdateCommandText, connection, transaction))
                    {
                        ...
                        sdistCommand.Parameters.AddWithValue("@SDistSelectId", model.SDistSelectId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Sinter process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the SDistSelect table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int SDistSelectId)
        {
            ...
            string commandText = "DELETE FROM SDistSelect WHERE SDistSelectId = @SDistSelectId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@SDistSelectId", SDistSelectId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted SDistSelect with ID {SDistSelectId}.");
