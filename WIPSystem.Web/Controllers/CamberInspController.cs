...
{
    public class CamberInspController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<CamberInspController> _logger;

        public CamberInspController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<CamberInspController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Camber Insp.' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT CamberInspId, PartNo, LotNo, OutputQty, Status 
            FROM CamberInsp 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<CamberInspViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new CamberInspViewModel
                        {
                            CamberInspId = reader.GetInt32(reader.GetOrdinal("CamberInspId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             CamberInspProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<CamberInspProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new CamberInspViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Camber Insp."
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(CamberInspViewModel model)
        {
        ...
        private void PrepareViewModel(CamberInspViewModel model)
        {
        ...
        public async Task<IActionResult> CamberInspDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            CamberInspViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE c.CamberInspId = @CamberInspId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@CamberInspId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new CamberInspViewModel
                        {
                            CamberInspId = reader.GetInt32(reader.GetOrdinal("CamberInspId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<CamberInspProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_CamberInspDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            CamberInspViewModel viewModel = new CamberInspViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.CamberInspId = reader.GetInt32(reader.GetOrdinal("CamberInspId"));
                            ...
                            if (Enum.TryParse<CamberInspProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = CamberInspProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(CamberInspViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(CamberInspViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE CamberInspId = @CamberInspId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@CamberInspId", model.CamberInspId);
        ...
        public async Task<IActionResult> Delete(int CamberInspId)
        {
            ...
            string commandText = "DELETE FROM CamberInsp WHERE CamberInspId = @CamberInspId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@CamberInspId", CamberInspId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted CamberInspId with ID {CamberInspId}.");
