...
{
    public class SendtoAMAController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<SendtoAMAController> _logger;

        public SendtoAMAController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<SendtoAMAController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Send to AMA' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT AMAId, PartNo, LotNo, OutputQty, Status 
            FROM AMA 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<SendToAmaViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new SendToAmaViewModel
                        {
                            AMAId = reader.GetInt32(reader.GetOrdinal("AMAId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             AMAProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<AMAProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new SendToAmaViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Send to AMA"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(SendToAmaViewModel model)
        {
        ...
        private void PrepareViewModel(SendToAmaViewModel model)
        {
        ...
        public async Task<IActionResult> SendToAMADetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            SendToAmaViewModel viewModel = null;
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                command.Parameters.AddWithValue("@AMAId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new SendToAmaViewModel
                        {
                            AMAId = reader.GetInt32(reader.GetOrdinal("AMAId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<AMAProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_SendToAMADetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            SendToAmaViewModel viewModel = new SendToAmaViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.AMAId = reader.GetInt32(reader.GetOrdinal("AMAId"));
                            ...
                            if (Enum.TryParse<AMAProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = AMAProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(SendToAmaViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(SendToAmaViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@AMAId", model.AMAId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "AMA process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the AMA table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int AMAId)
        {
            ...
            string commandText = "DELETE FROM AMA WHERE AMAId = @AMAId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@AMAId", AMAId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted AMA with ID {AMAId}.");
