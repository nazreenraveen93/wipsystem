...
namespace WIPSystem.Web.Controllers
{
    public class SQcController : Controller
    {
        ...
        private readonly ILogger<SQcController> _logger;

        public SQcController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<SQcController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'S.Qc' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT SQcId, PartNo, LotNo, OutputQty, Status 
            FROM SQc 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<SqcViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new SqcViewModel
                        {
                            SQcId = reader.GetInt32(reader.GetOrdinal("SQcId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             SqcProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<SqcProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new SqcViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "S.Qc"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(SqcViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the SQc table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a SQc record.");
        ...
        private void PrepareViewModel(SqcViewModel model)
        {
        ...
        public async Task<IActionResult> SQcDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            SqcViewModel viewModel = null;
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                command.Parameters.AddWithValue("@SQcId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new SqcViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<SqcProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_SQcDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            SqcViewModel viewModel = new SqcViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.SQcId = reader.GetInt32(reader.GetOrdinal("SQcId"));
                            ...
                            if (Enum.TryParse<SqcProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = SqcProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(SqcViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(SqcViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@SQcId", model.SQcId);
        ...
        public async Task<IActionResult> Delete(int SQcId)
        {
            ...
            string commandText = "DELETE FROM SQc WHERE SQcId = @SQcId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@SQcId", SQcId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted SQc with ID {SQcId}.");
