...
{
    public class ReScr4Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<ReScr4Controller> _logger;

        public ReScr4Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<ReScr4Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Re-Scr.4' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT ReScr4Id, PartNo, LotNo, OutputQty, Status 
            FROM ReScr4 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<ReScr4ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new ReScr4ViewModel
                        {
                            ReScr4Id = reader.GetInt32(reader.GetOrdinal("ReScr4Id")),
                            ...
                            ReScr4ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<ReScr4ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new ReScr4ViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Re-Scr.4"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(ReScr4ViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the ReScr4 table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a ReScr4 record.");
        ...
        private void PrepareViewModel(ReScr4ViewModel model)
        {
        ...
        public async Task<IActionResult> ReScr4Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            ReScr4ViewModel viewModel = null;
            ...
            WHERE r.ReScr4Id = @ReScr4Id";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@ReScr4Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new ReScr4ViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<ReScr4ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_ReScr4DetailsModal", viewModel);
        }
        ...
        {
            ReScr4ViewModel viewModel = new ReScr4ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.ReScr4Id = reader.GetInt32(reader.GetOrdinal("ReScr4Id"));
                            ...
                            if (Enum.TryParse<ReScr4ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = ReScr4ProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(ReScr4ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(ReScr4ViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var agCommand = new SqlCommand(agUpdateCommandText, connection, transaction))
                    {
                        ...
                        agCommand.Parameters.AddWithValue("@ReScr4Id", model.ReScr4Id);
        ...
        public async Task<IActionResult> Delete(int ReScr4Id)
        {
            ...
            string commandText = "DELETE FROM ReScr4 WHERE ReScr4Id = @ReScr4Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@ReScr4Id", ReScr4Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted ReScr4 with ID {ReScr4Id}.");
