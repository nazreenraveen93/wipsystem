...
{
    public class NiPlating3Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<NiPlating3Controller> _logger;

        public NiPlating3Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<NiPlating3Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Ni Plating3' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT NiPlating3Id, PartNo, LotNo, OutputQty, Status 
            FROM NiPlating3 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<NiPlating3ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new NiPlating3ViewModel
                        {
                            NiPlating3Id = reader.GetInt32(reader.GetOrdinal("NiPlating3Id")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             NiPlating3ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<NiPlating3ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new NiPlating3ViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Ni Plating3"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(NiPlating3ViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the NiPlating table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a NiPlating record.");
        ...
        private void PrepareViewModel(NiPlating3ViewModel model)
        {
        ...
        public async Task<IActionResult> NiPlating3Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            NiPlating3ViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE n.NiPlating3Id = @NiPlating3Id";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@NiPlating3Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new NiPlating3ViewModel
                        {
                            NiPlating3Id = reader.GetInt32(reader.GetOrdinal("NiPlating3Id")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<NiPlating3ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_NiPlating3DetailsModal", viewModel);
        }
        ...
        {
            NiPlating3ViewModel viewModel = new NiPlating3ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.NiPlating3Id = reader.GetInt32(reader.GetOrdinal("NiPlating3Id"));
                            ...
                            if (Enum.TryParse<NiPlating3ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                                viewModel.Status = parsedStatus;
                            }
                            else
                            {
                                viewModel.Status = NiPlating3ProcessStatus.OnHold; // Default value
                            }
        ...
        private async Task PrepareViewModelForEdit(NiPlating3ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(NiPlating3ViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE NiPlating3Id = @NiPlating3Id";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@NiPlating3Id", model.NiPlating3Id);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Ni Plating3 process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the Ni Plating3 table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int NiPlating3Id)
        {
            ...
            string commandText = "DELETE FROM NiPlating3 WHERE NiPlating3Id = @NiPlating3Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@NiPlating3Id", NiPlating3Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted NiPlating3 with ID {NiPlating3Id}.");
