...
namespace WIPSystem.Web.Controllers
{
    public class QAController : Controller
    {
        ...
        public QAController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<QAController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'QA' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT QAId, PartNo, LotNo, OutputQty, Status 
            FROM QA 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<QAViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new QAViewModel
                        {
                            QAId = reader.GetInt32(reader.GetOrdinal("QAId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             QAProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<QAProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new QAViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "QA"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(QAViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the QA table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a QA record.");
        ...
        private void PrepareViewModel(QAViewModel model)
        {
        ...
        public async Task<IActionResult> QADetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            QAViewModel viewModel = null;
            ...
            SELECT 
                ...
                p.PiecesPerBlank
            FROM QA q
            INNER JOIN Products p ON q.PartNo = p.PartNo
            WHERE q.QAId = @QAId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                command.Parameters.AddWithValue("@QAId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new QAViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<QAProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_QADetailsModal", viewModel);
        }
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            FROM QA q
            INNER JOIN Products p ON q.PartNo = p.PartNo 
            LEFT JOIN CurrentStatus cs ON q.PartNo = cs.PartNo AND q.LotNo = cs.LotNo
            WHERE q.QAId = @QAId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.QAId = reader.GetInt32(reader.GetOrdinal("QAId"));
                            ...
                            if (Enum.TryParse<QAProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = QAProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(QAViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(QAViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@QAId", model.QAId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "QA process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the QA table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int QAId)
        {
            ...
            string commandText = "DELETE FROM QA WHERE QAId = @QAId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@QAId", QAId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted QA with ID {QAId}.");
