...
{
    public class NiPlating4Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<NiPlating4Controller> _logger;

        public NiPlating4Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<NiPlating4Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Ni Plating4' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT NiPlating4Id, PartNo, LotNo, OutputQty, Status 
            FROM NiPlating4 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<NiPlating4ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new NiPlating4ViewModel
                        {
                            NiPlating4Id = reader.GetInt32(reader.GetOrdinal("NiPlating4Id")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             NiPlating4ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<NiPlating4ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new NiPlating4ViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Ni Plating4"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(NiPlating4ViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the NiPlating4 table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a NiPlating4 record.");
        ...
        private void PrepareViewModel(NiPlating4ViewModel model)
        {
        ...
        public async Task<IActionResult> NiPlating4Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            NiPlating4ViewModel viewModel = null;
            ...
            string commandText = @"
                SELECT 
                    ...
                    p.PiecesPerBlank
                FROM NiPlating4 n
                INNER JOIN Machines m ON n.MachineId = m.MachineId
                INNER JOIN Products p ON n.PartNo = p.PartNo
                WHERE n.NiPlating4Id = @NiPlating4Id";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@NiPlating4Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new NiPlating4ViewModel
                        {
                            NiPlating4Id = reader.GetInt32(reader.GetOrdinal("NiPlating4Id")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<NiPlating4ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_NiPlating4DetailsModal", viewModel);
        }
        ...
        {
            NiPlating4ViewModel viewModel = new NiPlating4ViewModel();
        ...
        INNER JOIN Machines m ON n.MachineId = m.MachineId
        INNER JOIN Products p ON n.PartNo = p.PartNo 
        LEFT JOIN CurrentStatus cs ON n.PartNo = cs.PartNo AND n.LotNo = cs.LotNo
        WHERE n.NiPlating4Id = @NiPlating4Id";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.NiPlating4Id = reader.GetInt32(reader.GetOrdinal("NiPlating4Id"));
                            ...
                            if (Enum.TryParse<NiPlating4ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                                viewModel.Status = parsedStatus;
                            }
                            else
                            {
                                viewModel.Status = NiPlating4ProcessStatus.OnHold; // Default value
                            }
        ...
        private async Task PrepareViewModelForEdit(NiPlating4ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(NiPlating4ViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE NiPlating4Id = @NiPlating4Id";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@NiPlating4Id", model.NiPlating4Id);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Ni Plating4 process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the Ni Plating4 table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int NiPlating4Id)
        {
            ...
            string commandText = "DELETE FROM NiPlating4 WHERE NiPlating4Id = @NiPlating4Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@NiPlating4Id", NiPlating4Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted NiPlating4 with ID {NiPlating4Id}.");
