...
{
    public class ReScr5Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<ReScr5Controller> _logger;

        public ReScr5Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<ReScr5Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Re-Scr.5' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT ReScr5Id, PartNo, LotNo, OutputQty, Status 
            FROM ReScr5 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<ReScr5ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new ReScr5ViewModel
                        {
                            ReScr5Id = reader.GetInt32(reader.GetOrdinal("ReScr5Id")),
                            ...
                            ReScr5ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<ReScr5ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new ReScr5ViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Re-Scr.5"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(ReScr5ViewModel model)
        {
        ...
        private void PrepareViewModel(ReScr5ViewModel model)
        {
        ...
        public async Task<IActionResult> ReScr5Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            ReScr5ViewModel viewModel = null;
            ...
            WHERE r.ReScr5Id = @ReScr5Id";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@ReScr5Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new ReScr5ViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<ReScr5ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_ReScr5DetailsModal", viewModel);
        }
        ...
        {
            ReScr5ViewModel viewModel = new ReScr5ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.ReScr5Id = reader.GetInt32(reader.GetOrdinal("ReScr5Id"));
                            ...
                            if (Enum.TryParse<ReScr5ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = ReScr5ProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(ReScr5ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(ReScr5ViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var agCommand = new SqlCommand(agUpdateCommandText, connection, transaction))
                    {
                        ...
                        agCommand.Parameters.AddWithValue("@ReScr5Id", model.ReScr5Id);
        ...
        public async Task<IActionResult> Delete(int ReScr5Id)
        {
            ...
            string commandText = "DELETE FROM ReScr5 WHERE ReScr5Id = @ReScr5Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@ReScr5Id", ReScr5Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted ReScr5 with ID {ReScr5Id}.");
