...
{
    public class SubConOutController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<SubConOutController> _logger;

        public SubConOutController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<SubConOutController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'S-Subcon-Out' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT SSubconOutId, PartNo, LotNo, OutputQty, Status 
            FROM SSubconOut 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<SubOutViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new SubOutViewModel
                        {
                            SSubconOutId = reader.GetInt32(reader.GetOrdinal("SSubconOutId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             SSubconOutProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<SSubconOutProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new SubOutViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "S-Subcon-Out"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(SubOutViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the SSubconOut table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a SSubconOut record.");
        ...
        private void PrepareViewModel(SubOutViewModel model)
        {
        ...
        public async Task<IActionResult> SubConOutDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            SubOutViewModel viewModel = null;
            ...
            WHERE s.SSubconOutId = @SSubconOutId";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@SSubconOutId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new SubOutViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<SSubconOutProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_SubConOutDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            SubOutViewModel viewModel = new SubOutViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.SSubconOutId = reader.GetInt32(reader.GetOrdinal("SSubconOutId"));
                            ...
                            if (Enum.TryParse<SSubconOutProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = SSubconOutProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(SubOutViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(SubOutViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            WHERE SSubconOutId = @SSubconOutId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@SSubconOutId", model.SSubconOutId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "SSubconOut process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the SSubconOut table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int SSubconOutId)
        {
            ...
            string commandText = "DELETE FROM SSubconOut WHERE SSubconOutId = @SSubconOutId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@SSubconOutId", SSubconOutId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted SSubconOut with ID {SSubconOutId}.");
