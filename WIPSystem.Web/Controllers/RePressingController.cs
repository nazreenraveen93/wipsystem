...
{
    public class RePressingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<RePressingController> _logger;

        public RePressingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<RePressingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Re-Pressing' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT RePressingId, PartNo, LotNo, OutputQty, Status 
            FROM RePressing 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<RePressingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new RePressingViewModel
                        {
                            RePressingId = reader.GetInt32(reader.GetOrdinal("RePressingId")),
                            ...
                            RePressingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<RePressingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new RePressingViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Re-Pressing"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(RePressingViewModel model)
        {
        ...
        private void PrepareViewModel(RePressingViewModel model)
        {
        ...
        public async Task<IActionResult> RePressingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            RePressingViewModel viewModel = null;
            ...
            WHERE r.RePressingId = @RePressingId";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@RePressingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new RePressingViewModel
                        {
                            RePressingId = reader.GetInt32(reader.GetOrdinal("RePressingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<RePressingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_RePressingDetailsModal", viewModel);
        }
        ...
        {
            RePressingViewModel viewModel = new RePressingViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.RePressingId = reader.GetInt32(reader.GetOrdinal("RePressingId"));
                            ...
                            if (Enum.TryParse<RePressingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = RePressingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(RePressingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(RePressingViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            WHERE RePressingId = @RePressingId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var agCommand = new SqlCommand(agUpdateCommandText, connection, transaction))
                    {
                        ...
                        agCommand.Parameters.AddWithValue("@RePressingId", model.RePressingId);
        ...
        public async Task<IActionResult> Delete(int RePressingId)
        {
            ...
            string commandText = "DELETE FROM RePressing WHERE RePressingId = @RePressingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@RePressingId", RePressingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted RePressing with ID {RePressingId}.");
