...
{
    public class AgcusetController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<AgcusetController> _logger;

        public AgcusetController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<AgcusetController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Agcuset' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT AgcusetId, PartNo, LotNo, OutputQty, Status 
            FROM Agcuset 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<AgcusetViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new AgcusetViewModel
                        {
                            AgcusetId = reader.GetInt32(reader.GetOrdinal("AgcusetId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             AgcusetProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<AgcusetProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new AgcusetViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Agcuset"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(AgcusetViewModel model)
        {
        ...
        private void PrepareViewModel(AgcusetViewModel model)
        {
        ...
        public async Task<IActionResult> AgcusetDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            AgcusetViewModel viewModel = null;
            ...
            WHERE a.AgcusetId = @AgcusetId";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@AgcusetId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new AgcusetViewModel
                        {
                            AgcusetId = reader.GetInt32(reader.GetOrdinal("AgcusetId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<AgcusetProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_AgcusetDetailsModal", viewModel);
        }
        ...
        {
            AgcusetViewModel viewModel = new AgcusetViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.AgcusetId = reader.GetInt32(reader.GetOrdinal("AgcusetId"));
                            ...
                            if (Enum.TryParse<AgcusetProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = AgcusetProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(AgcusetViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(AgcusetViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var agCommand = new SqlCommand(agUpdateCommandText, connection, transaction))
                    {
                        ...
                        agCommand.Parameters.AddWithValue("@AgcusetId", model.AgcusetId);
