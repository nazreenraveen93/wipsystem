...
{
    public class Insp5Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<Insp5Controller> _logger;

        public Insp5Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<Insp5Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Insp.5' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT Insp5Id, PartNo, LotNo, OutputQty, Status 
            FROM Insp5 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<Insp5ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new Insp5ViewModel
                        {
                            Insp5Id = reader.GetInt32(reader.GetOrdinal("Insp5Id")),
                            ...
                            Insp5ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<Insp5ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new Insp5ViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Insp.5"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Insp5ViewModel model)
        {
                ...
                using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a Insp4 record.");
        ...
        private void PrepareViewModel(Insp5ViewModel model)
        {
        ...
        public async Task<IActionResult> Insp5Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            Insp5ViewModel viewModel = null;
            ...
            WHERE i.Insp5Id = @Insp5Id";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@Insp5Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new Insp5ViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<Insp5ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_Insp5DetailsModal", viewModel);
        }
        ...
        {
            Insp5ViewModel viewModel = new Insp5ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.Insp5Id = reader.GetInt32(reader.GetOrdinal("Insp5Id"));
                            ...
                            if (Enum.TryParse<Insp5ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = Insp5ProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(Insp5ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(Insp5ViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var agCommand = new SqlCommand(agUpdateCommandText, connection, transaction))
                    {
                        ...
                        agCommand.Parameters.AddWithValue("@Insp5Id", model.Insp5Id);
        ...
        public async Task<IActionResult> Delete(int Insp5Id)
        {
            ...
            string commandText = "DELETE FROM Insp5 WHERE Insp5Id = @Insp5Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@Insp5Id", Insp5Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted Insp5 with ID {Insp5Id}.");
