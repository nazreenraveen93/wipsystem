...
{
    public class SGrindingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<SGrindingController> _logger;

        public SGrindingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<SGrindingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'S-Grinding' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT SGrindingId, PartNo, LotNo, OutputQty, Status 
            FROM SGrinding 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<SGrindingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new SGrindingViewModel
                        {
                            SGrindingId = reader.GetInt32(reader.GetOrdinal("SGrindingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             SGrindingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<SGrindingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new SGrindingViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "S-Grinding"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(SGrindingViewModel model)
        {
        ...
        private void PrepareViewModel(SGrindingViewModel model)
        {
        ...
        public async Task<IActionResult> SGrindingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            SGrindingViewModel viewModel = null;
            ...
            WHERE s.SGrindingId = @SGrindingId";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@SGrindingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new SGrindingViewModel
                        {
                            SGrindingId = reader.GetInt32(reader.GetOrdinal("SGrindingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<SGrindingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_SGrindingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            SGrindingViewModel viewModel = new SGrindingViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.SGrindingId = reader.GetInt32(reader.GetOrdinal("SGrindingId"));
                            ...
                            if (Enum.TryParse<SGrindingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = SGrindingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(SGrindingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(SGrindingViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@SGrindingId", model.SGrindingId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "SGrinding process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the SGrinding table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int SGrindingId)
        {
            ...
            string commandText = "DELETE FROM SGrinding WHERE SGrindingId = @SGrindingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@SGrindingId", SGrindingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted SGrindingId with ID {SGrindingId}.");
