...
{
    public class AuPlating2Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<AuPlating2Controller> _logger;

        public AuPlating2Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<AuPlating2Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Au Plating2' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT AuPlating2Id, PartNo, LotNo, OutputQty, Status 
            FROM AuPlating2 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<AuPlating2ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new AuPlating2ViewModel
                        {
                            AuPlating2Id = reader.GetInt32(reader.GetOrdinal("AuPlating2Id")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             AuPlating2ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<AuPlating2ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new AuPlating2ViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Au Plating2"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(AuPlating2ViewModel model)
        {
        ...
        private void PrepareViewModel(AuPlating2ViewModel model)
        {
        ...
        public async Task<IActionResult> AuPlating2Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            AuPlating2ViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE a.AuPlating2Id = @AuPlating2Id";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@AuPlating2Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new AuPlating2ViewModel
                        {
                            AuPlating2Id = reader.GetInt32(reader.GetOrdinal("AuPlating2Id")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<AuPlating2ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_AuPlating2DetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            AuPlating2ViewModel viewModel = new AuPlating2ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.AuPlating2Id = reader.GetInt32(reader.GetOrdinal("AuPlating2Id"));
                            ...
                            if (Enum.TryParse<AuPlating2ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = AuPlating2ProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(AuPlating2ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(AuPlating2ViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE AuPlating2Id = @AuPlating2Id";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@AuPlating2Id", model.AuPlating2Id);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "AuPlating2 process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the AuPlating2 table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int AuPlating2Id)
        {
            ...
            string commandText = "DELETE FROM AuPlating2 WHERE AuPlating2Id = @AuPlating2Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@AuPlating2Id", AuPlating2Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted AuPlating2Id with ID {AuPlating2Id}.");
