...
{
    public class LeadCuttingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<LeadCuttingController> _logger;

        public LeadCuttingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<LeadCuttingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Lead Cutting' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT LeadCuttingId, PartNo, LotNo, OutputQty, Status 
            FROM LeadCutting 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<LeadCuttingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new LeadCuttingViewModel
                        {
                            LeadCuttingId = reader.GetInt32(reader.GetOrdinal("LeadCuttingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             LeadCuttingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<LeadCuttingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new LeadCuttingViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Lead Cutting"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(LeadCuttingViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the Lead Cutting table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a LeadCutting record.");
        ...
        private void PrepareViewModel(LeadCuttingViewModel model)
        {
        ...
        public async Task<IActionResult> LeadCuttingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            LeadCuttingViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                INNER JOIN Products p ON l.PartNo = p.PartNo
                WHERE l.LeadCuttingId = @LeadCuttingId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@LeadCuttingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new LeadCuttingViewModel
                        {
                            LeadCuttingId = reader.GetInt32(reader.GetOrdinal("LeadCuttingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<LeadCuttingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_LeadCuttingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            LeadCuttingViewModel viewModel = new LeadCuttingViewModel();
            ...
            INNER JOIN Machines m ON l.MachineId = m.MachineId
            INNER JOIN Products p ON l.PartNo = p.PartNo 
            LEFT JOIN CurrentStatus cs ON l.PartNo = cs.PartNo AND l.LotNo = cs.LotNo
            WHERE l.LeadCuttingId = @LeadCuttingId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.LeadCuttingId = reader.GetInt32(reader.GetOrdinal("LeadCuttingId"));
                            ...
                            if (Enum.TryParse<LeadCuttingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = LeadCuttingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(LeadCuttingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(LeadCuttingViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE LeadCuttingId = @LeadCuttingId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@LeadCuttingId", model.LeadCuttingId);
        ...
        public async Task<IActionResult> Delete(int LeadCuttingId)
        {
            ...
            string commandText = "DELETE FROM LeadCutting WHERE LeadCuttingId = @LeadCuttingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@LeadCuttingId", LeadCuttingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted LeadCuttingId with ID {LeadCuttingId}.");
