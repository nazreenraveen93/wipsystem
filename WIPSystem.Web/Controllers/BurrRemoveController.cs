...
{
    public class BurrRemoveController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<BurrRemoveController> _logger;

        public BurrRemoveController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<BurrRemoveController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Burr Remove' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT BurrRemoveId, PartNo, LotNo, OutputQty, Status 
            FROM BurrRemove 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<BurrRemoveViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new BurrRemoveViewModel
                        {
                            BurrRemoveId = reader.GetInt32(reader.GetOrdinal("BurrRemoveId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             BurrRemoveProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<BurrRemoveProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new BurrRemoveViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Burr Remove"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(BurrRemoveViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the BurrRemove table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a BurrRemove record.");
        ...
        private void PrepareViewModel(BurrRemoveViewModel model)
        {
        ...
        public async Task<IActionResult> BurrRemoveDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            BurrRemoveViewModel viewModel = null;
            ...
            WHERE b.BurrRemoveId = @BurrRemoveId";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@BurrRemoveId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new BurrRemoveViewModel
                        {
                            BurrRemoveId = reader.GetInt32(reader.GetOrdinal("BurrRemoveId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<BurrRemoveProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_BurrRemoveDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            BurrRemoveViewModel viewModel = new BurrRemoveViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.BurrRemoveId = reader.GetInt32(reader.GetOrdinal("BurrRemoveId"));
                            ...
                            if (Enum.TryParse<BurrRemoveProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = BurrRemoveProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(BurrRemoveViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(BurrRemoveViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE BurrRemoveId = @BurrRemoveId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@BurrRemoveId", model.BurrRemoveId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "BurrRemove process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
        ...
        public async Task<IActionResult> Delete(int BurrRemoveId)
        {
            ...
            string commandText = "DELETE FROM BurrRemove WHERE BurrRemoveId = @BurrRemoveId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@BurrRemoveId", BurrRemoveId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted SDistSelect with ID {BurrRemoveId}.");
