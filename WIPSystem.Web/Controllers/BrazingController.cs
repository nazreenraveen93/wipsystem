...
{
    public class BrazingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<BrazingController> _logger;

        public BrazingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<BrazingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Brazing' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT BrazingId, PartNo, LotNo, OutputQty, Status 
            FROM Brazing 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<BrazingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new BrazingViewModel
                        {
                            BrazingId = reader.GetInt32(reader.GetOrdinal("BrazingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             BrazingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<BrazingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new BrazingViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Brazing"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(BrazingViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the Brazing table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a Brazing record.");
        ...
        private void PrepareViewModel(BrazingViewModel model)
        {
        ...
        public async Task<IActionResult> BrazingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            BrazingViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE b.BrazingId = @BrazingId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@BrazingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new BrazingViewModel
                        {
                            BrazingId = reader.GetInt32(reader.GetOrdinal("BrazingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<BrazingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_BrazingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            BrazingViewModel viewModel = new BrazingViewModel();
            ...
            INNER JOIN Machines m ON b.MachineId = m.MachineId
            INNER JOIN Products p ON b.PartNo = p.PartNo 
            LEFT JOIN CurrentStatus cs ON b.PartNo = cs.PartNo AND b.LotNo = cs.LotNo
            WHERE b.BrazingId = @BrazingId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.BrazingId = reader.GetInt32(reader.GetOrdinal("BrazingId"));
                            ...
                            if (Enum.TryParse<BrazingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = BrazingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(BrazingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(BrazingViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sdistCommand = new SqlCommand(SDistUpdateCommandText, connection, transaction))
                    {
                        ...
                        sdistCommand.Parameters.AddWithValue("@BrazingId", model.BrazingId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Brazing process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the Brazing table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int BrazingId)
        {
            ...
            string commandText = "DELETE FROM Brazing WHERE BrazingId = @BrazingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@BrazingId", BrazingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted Brazing with ID {BrazingId}.");
