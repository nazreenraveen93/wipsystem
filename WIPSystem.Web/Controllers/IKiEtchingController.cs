...
{
    public class IKiEtchingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<IKiEtchingController> _logger;

        public IKiEtchingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<IKiEtchingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'I-Ki Etching' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT IKiEtchingId, PartNo, LotNo, OutputQty, Status 
            FROM IKiEtching 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<IKiEtchingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new IKiEtchingViewModel
                        {
                            IKiEtchingId = reader.GetInt32(reader.GetOrdinal("IKiEtchingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             IKiEtchingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<IKiEtchingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new IKiEtchingViewModel
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(IKiEtchingViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the IKiEtching table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a IKiEtching record.");
        ...
        private void PrepareViewModel(IKiEtchingViewModel model)
        {
        ...
        public async Task<IActionResult> IKiEtchingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            IKiEtchingViewModel viewModel = null;
            ...
            string commandText = @"
                SELECT 
                    ...
                    p.PiecesPerBlank
                FROM IKiEtching i
                INNER JOIN Machines m ON i.MachineId = m.MachineId
                INNER JOIN Products p ON i.PartNo = p.PartNo
                WHERE i.IKiEtchingId = @IKiEtchingId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@IKiEtchingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new IKiEtchingViewModel
                        {
                            IKiEtchingId = reader.GetInt32(reader.GetOrdinal("IKiEtchingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<IKiEtchingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_IKiEtchingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            IKiEtchingViewModel viewModel = new IKiEtchingViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.IKiEtchingId = reader.GetInt32(reader.GetOrdinal("IKiEtchingId"));
                            ...
                            if (Enum.TryParse<IKiEtchingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = IKiEtchingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(IKiEtchingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(IKiEtchingViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE IKiEtchingId = @IKiEtchingId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sdistCommand = new SqlCommand(SDistUpdateCommandText, connection, transaction))
                    {
                        ...
                        sdistCommand.Parameters.AddWithValue("@IKiEtchingId", model.IKiEtchingId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "IKiEtching process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the IKiEtching table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int IKiEtchingId)
        {
            ...
            string commandText = "DELETE FROM IKiEtching WHERE IKiEtchingId = @IKiEtchingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@IKiEtchingId", IKiEtchingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted IKiEtching with ID {IKiEtchingId}.");
