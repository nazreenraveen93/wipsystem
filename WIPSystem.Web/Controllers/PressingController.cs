...
namespace WIPSystem.Web.Controllers
{
    public class PressingController : Controller
    {
        ...
        private readonly ILogger<PressingController> _logger;

        public PressingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<PressingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Pressing' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT PressingId, PartNo, LotNo, OutputQty, Status 
            FROM Pressing 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<PressingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new PressingViewModel
                        {
                            PressingId = reader.GetInt32(reader.GetOrdinal("PressingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             PressingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<PressingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new PressingViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Pressing"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(PressingViewModel model)
        {
        ...
        private void PrepareViewModel(PressingViewModel model)
        {
        ...
        public async Task<IActionResult> PressingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            PressingViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                INNER JOIN Products p ON pr.PartNo = p.PartNo
                WHERE pr.PressingId = @PressingId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@PressingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new PressingViewModel
                        {
                            PressingId = reader.GetInt32(reader.GetOrdinal("PressingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<PressingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_PressingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            PressingViewModel viewModel = new PressingViewModel();
            ...
            INNER JOIN Machines m ON pr.MachineId = m.MachineId
            INNER JOIN Products p ON pr.PartNo = p.PartNo 
            LEFT JOIN CurrentStatus cs ON pr.PartNo = cs.PartNo AND pr.LotNo = cs.LotNo
            WHERE pr.PressingId = @PressingId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.PressingId = reader.GetInt32(reader.GetOrdinal("PressingId"));
                            ...
                            if (Enum.TryParse<PressingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = PressingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(PressingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(PressingViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@PressingId", model.PressingId);
        ...
        public async Task<IActionResult> Delete(int PressingId)
        {
            ...
            string commandText = "DELETE FROM Ni Pressing WHERE PressingId = @PressingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@PressingId", PressingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted PressingId with ID {PressingId}.");
