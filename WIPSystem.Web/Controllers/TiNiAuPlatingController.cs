...
{
    public class TiNiAuPlatingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<TiNiAuPlatingController> _logger;

        public TiNiAuPlatingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<TiNiAuPlatingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Ti Ni Au Plating' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT TiNiAuPlatingId, PartNo, LotNo, OutputQty, Status 
            FROM TiNiAuPlating 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<TiNiAuPlatingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new TiNiAuPlatingViewModel
                        {
                            TiNiAuPlatingId = reader.GetInt32(reader.GetOrdinal("TiNiAuPlatingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             TiNiAuPlatingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<TiNiAuPlatingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new TiNiAuPlatingViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Ti Ni Au Plating"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(TiNiAuPlatingViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the Cleaning table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a Cleaning record.");
        ...
        private void PrepareViewModel(TiNiAuPlatingViewModel model)
        {
        ...
        public async Task<IActionResult> TiNiAuPlatingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            TiNiAuPlatingViewModel viewModel = null;
            ...
            string commandText = @"
                SELECT 
                    ...
                    p.PackageSize,
                    p.PiecesPerBlank
                FROM TiNiAuPlating t
                INNER JOIN Machines m ON t.MachineId = m.MachineId
                INNER JOIN Products p ON t.PartNo = p.PartNo
                WHERE t.TiNiAuPlatingId = @TiNiAuPlatingId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@TiNiAuPlatingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new TiNiAuPlatingViewModel
                        {
                            TiNiAuPlatingId = reader.GetInt32(reader.GetOrdinal("TiNiAuPlatingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<TiNiAuPlatingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_TiNiAuPlatingDetailsModal", viewModel);
        }
        ...
        {
            TiNiAuPlatingViewModel viewModel = new TiNiAuPlatingViewModel();
            ...
            INNER JOIN Machines m ON t.MachineId = m.MachineId
            INNER JOIN Products p ON t.PartNo = p.PartNo 
            LEFT JOIN CurrentStatus cs ON t.PartNo = cs.PartNo AND t.LotNo = cs.LotNo
            WHERE t.TiNiAuPlatingId = @TiNiAuPlatingId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.TiNiAuPlatingId = reader.GetInt32(reader.GetOrdinal("TiNiAuPlatingId"));
                            ...
                            if (Enum.TryParse<TiNiAuPlatingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = TiNiAuPlatingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(TiNiAuPlatingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(TiNiAuPlatingViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@TiNiAuPlatingId", model.TiNiAuPlatingId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "AupdPlating process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the AupdPlating table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int TiNiAuPlatingId)
        {
            ...
            string commandText = "DELETE FROM TiNiAuPlating WHERE TiNiAuPlatingId = @TiNiAuPlatingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@TiNiAuPlatingId", TiNiAuPlatingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted TiNiAuPlating with ID {TiNiAuPlatingId}.");
