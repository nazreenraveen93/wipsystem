...
namespace WIPSystem.Web.Controllers
{
    public class CleaningController : Controller
    {
        ...
        public CleaningController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<CleaningController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Cleaning' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT CleaningId, PartNo, LotNo, OutputQty, Status 
            FROM Cleaning 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<CleaningViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new CleaningViewModel
                        {
                            CleaningId = reader.GetInt32(reader.GetOrdinal("CleaningId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             CleaningProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<CleaningProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        [HttpPost]
        public async Task<IActionResult> Create(CleaningViewModel model)
        {
        ...
        private void PrepareViewModel(CleaningViewModel model)
        {
        ...
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            CleaningViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE c.CleaningId = @CleaningId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@CleaningId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new CleaningViewModel
                        {
                            CleaningId = reader.GetInt32(reader.GetOrdinal("CleaningId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<CleaningProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_CleaningDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            CleaningViewModel viewModel = new CleaningViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.CleaningId = reader.GetInt32(reader.GetOrdinal("CleaningId"));
                            ...
                            if (Enum.TryParse<CleaningProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = CleaningProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(CleaningViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(CleaningViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@CleaningId", model.CleaningId);
        ...
        public async Task<IActionResult> Delete(int CleaningId)
        {
            ...
            string commandText = "DELETE FROM Cleaning WHERE CleaningId = @CleaningId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        _logger.LogInformation($"Deleted Cleaning with ID {CleaningId}.");
