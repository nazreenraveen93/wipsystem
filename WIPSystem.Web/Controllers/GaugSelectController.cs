...
{
    public class GaugSelectController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<GaugSelectController> _logger;

        public GaugSelectController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<GaugSelectController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Gaug Select' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT GaugSelectId, PartNo, LotNo, OutputQty, Status 
            FROM GaugSelect 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<GaugSelectViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new GaugSelectViewModel
                        {
                            GaugSelectId = reader.GetInt32(reader.GetOrdinal("GaugSelectId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             GaugSelectProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<GaugSelectProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new GaugSelectViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Gaug Select"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(GaugSelectViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the Agcuset table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a Agcuset record.");
        ...
        private void PrepareViewModel(GaugSelectViewModel model)
        {
        ...
        public async Task<IActionResult> GaugSelectDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            GaugSelectViewModel viewModel = null;
            ...
            SELECT 
                ...
                p.PiecesPerBlank
            FROM GaugSelect g
            INNER JOIN Products p ON g.PartNo = p.PartNo
            WHERE g.GaugSelectId = @GaugSelectId";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@GaugSelectId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new GaugSelectViewModel
                        {
                            GaugSelectId = reader.GetInt32(reader.GetOrdinal("GaugSelectId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<GaugSelectProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_GaugSelectDetailsModal", viewModel);
        }
        ...
        {
            GaugSelectViewModel viewModel = new GaugSelectViewModel();
            ...
            FROM GaugSelect g
            INNER JOIN Products p ON g.PartNo = p.PartNo 
            LEFT JOIN CurrentStatus cs ON g.PartNo = cs.PartNo AND g.LotNo = cs.LotNo
            WHERE g.GaugSelectId = @GaugSelectId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.GaugSelectId = reader.GetInt32(reader.GetOrdinal("GaugSelectId"));
                            ...
                            if (Enum.TryParse<GaugSelectProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = GaugSelectProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(GaugSelectViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(GaugSelectViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            WHERE GaugSelectId = @GaugSelectId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var agCommand = new SqlCommand(agUpdateCommandText, connection, transaction))
                    {
                        ...
                        agCommand.Parameters.AddWithValue("@GaugSelectId", model.GaugSelectId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Agcuset process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the BurrRemove table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int GaugSelectId)
        {
            ...
            string commandText = "DELETE FROM GaugSelect WHERE GaugSelectId = @GaugSelectId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@GaugSelectId", GaugSelectId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted GaugSelect with ID {GaugSelectId}.");
