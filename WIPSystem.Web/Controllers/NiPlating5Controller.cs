...
{
    public class NiPlating5Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<NiPlating5Controller> _logger;

        public NiPlating5Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<NiPlating5Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Ni Plating5' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT NiPlating5Id, PartNo, LotNo, OutputQty, Status 
            FROM NiPlating5 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<NiPlating5ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new NiPlating5ViewModel
                        {
                            NiPlating5Id = reader.GetInt32(reader.GetOrdinal("NiPlating5Id")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             NiPlating5ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<NiPlating5ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new NiPlating5ViewModel
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(NiPlating5ViewModel model)
        {
        ...
        private void PrepareViewModel(NiPlating5ViewModel model)
        {
        ...
        public async Task<IActionResult> NiPlating5Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            NiPlating5ViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE n.NiPlating5Id = @NiPlating5Id";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@NiPlating5Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new NiPlating5ViewModel
                        {
                            NiPlating5Id = reader.GetInt32(reader.GetOrdinal("NiPlating5Id")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<NiPlating5ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_NiPlating5DetailsModal", viewModel);
        }
        ...
        {
            NiPlating5ViewModel viewModel = new NiPlating5ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.NiPlating5Id = reader.GetInt32(reader.GetOrdinal("NiPlating5Id"));
                            ...
                            if (Enum.TryParse<NiPlating5ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                                viewModel.Status = parsedStatus;
                            }
                            else
                            {
                                viewModel.Status = NiPlating5ProcessStatus.OnHold; // Default value
                            }
        ...
        private async Task PrepareViewModelForEdit(NiPlating5ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(NiPlating5ViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE NiPlating5Id = @NiPlating5Id";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@NiPlating5Id", model.NiPlating5Id);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Ni Plating5 process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the Ni Plating5 table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int NiPlating5Id)
        {
            ...
            string commandText = "DELETE FROM NiPlating5 WHERE NiPlating5Id = @NiPlating5Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@NiPlating5Id", NiPlating5Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted NiPlating5 with ID {NiPlating5Id}.");
