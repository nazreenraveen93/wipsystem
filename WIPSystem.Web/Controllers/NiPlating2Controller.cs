...
{
    public class NiPlating2Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<NiPlating2Controller> _logger;

        public NiPlating2Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<NiPlating2Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Ni Plating2' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT NiPlating2Id, PartNo, LotNo, OutputQty, Status 
            FROM NiPlating2 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<NiPlating2ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new NiPlating2ViewModel
                        {
                            NiPlating2Id = reader.GetInt32(reader.GetOrdinal("NiPlating2Id")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             NiPlating2ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<NiPlating2ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new NiPlating2ViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Ni Plating2"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(NiPlating2ViewModel model)
        {
        ...
        private void PrepareViewModel(NiPlating2ViewModel model)
        {
        ...
        public async Task<IActionResult> NiPlating2Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            NiPlating2ViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE n.NiPlating2Id = @NiPlating2Id";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@NiPlating2Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new NiPlating2ViewModel
                        {
                            NiPlating2Id = reader.GetInt32(reader.GetOrdinal("NiPlating2Id")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<NiPlating2ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_NiPlating2DetailsModal", viewModel);
        }
        ...
        {
            NiPlating2ViewModel viewModel = new NiPlating2ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.NiPlating2Id = reader.GetInt32(reader.GetOrdinal("NiPlating2Id"));
                            ...
                            if (Enum.TryParse<NiPlating2ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                                viewModel.Status = parsedStatus;
                            }
                            else
                            {
                                viewModel.Status = NiPlating2ProcessStatus.OnHold; // Default value
                            }
        ...
        private async Task PrepareViewModelForEdit(NiPlating2ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(NiPlating2ViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE NiPlating2Id = @NiPlating2Id";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@NiPlating2Id", model.NiPlating2Id);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Ni Plating process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the Ni Plating table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int NiPlating2Id)
        {
            ...
            string commandText = "DELETE FROM NiPlating2 WHERE NiPlating2Id = @NiPlating2Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@NiPlating2Id", NiPlating2Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted NiPlating2 with ID {NiPlating2Id}.");
