...
{
    public class Sinter2Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<Sinter2Controller> _logger;

        public Sinter2Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<Sinter2Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = '750Sinter2' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT Sinter2Id, PartNo, LotNo, OutputQty, Status 
            FROM Sinter2 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<Sinter2ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new Sinter2ViewModel
                        {
                            Sinter2Id = reader.GetInt32(reader.GetOrdinal("Sinter2Id")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             Sinter2ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<Sinter2ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new Sinter2ViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "750Sinter2"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Sinter2ViewModel model)
        {
        ...
        private void PrepareViewModel(Sinter2ViewModel model)
        {
        ...
        public async Task<IActionResult> Sinter2Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            Sinter2ViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE s.Sinter2Id = @Sinter2Id";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@Sinter2Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new Sinter2ViewModel
                        {
                            Sinter2Id = reader.GetInt32(reader.GetOrdinal("Sinter2Id")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<Sinter2ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_Sinter2DetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            Sinter2ViewModel viewModel = new Sinter2ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.Sinter2Id = reader.GetInt32(reader.GetOrdinal("Sinter2Id"));
                            ...
                            if (Enum.TryParse<Sinter2ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = Sinter2ProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(Sinter2ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(Sinter2ViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(sinterUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@Sinter2Id", model.Sinter2Id);
        ...
        public async Task<IActionResult> Delete(int Sinter2Id)
        {
            ...
            string commandText = "DELETE FROM Sinter2 WHERE Sinter2Id = @Sinter2Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@Sinter2Id", Sinter2Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted Sinter2 with ID {Sinter2Id}.");
