...
namespace WIPSystem.Web.Controllers
{
    public class StorageController : Controller
    {
        ...
        public StorageController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<StorageController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Storage' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT StorageId, PartNo, LotNo, OutputQty, Status 
            FROM Storage 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<StorageViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                    ...
                    while (reader.Read())
                    {
                        var item = new StorageViewModel
                        {
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             StorageProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<StorageProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        private void PrepareViewModel(StorageViewModel model)
        {
        ...
        public async Task<IActionResult> StorageDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            StorageViewModel viewModel = null;
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                command.Parameters.AddWithValue("@StorageId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new StorageViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<StorageProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_StorageDetailsModal", viewModel);
        }
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.StorageId = reader.GetInt32(reader.GetOrdinal("StorageId"));
                            ...
                            if (Enum.TryParse<StorageProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = StorageProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(StorageViewModel model)
        {
            // Retrieve the process ID based on the current status name.
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@StorageId", model.StorageId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Storage process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the Storage table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int StorageId)
        {
            ...
            string commandText = "DELETE FROM Storage WHERE StorageId = @StorageId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        _logger.LogInformation($"Deleted Storage with ID {StorageId}.");
