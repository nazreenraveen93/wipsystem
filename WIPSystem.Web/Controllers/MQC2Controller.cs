...
{
    public class MQC2Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<MQC2Controller> _logger;

        public MQC2Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<MQC2Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'MQC2' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT MQC2Id, PartNo, LotNo, OutputQty, Status 
            FROM MQC2 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<MQC2ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new MQC2ViewModel
                        {
                            MQC2Id = reader.GetInt32(reader.GetOrdinal("MQC2Id")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             MQC2ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<MQC2ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new MQC2ViewModel

            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "MQC2"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(MQC2ViewModel model)
        {
        ...
        private void PrepareViewModel(MQC2ViewModel model)
        {
        ...
        public async Task<IActionResult> MQC2Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            MQC2ViewModel viewModel = null;
            ...
            WHERE m.MQC2Id = @MQC2Id";


            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@MQC2Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new MQC2ViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<MQC2ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_MQC2DetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            MQC2ViewModel viewModel = new MQC2ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.MQC2Id = reader.GetInt32(reader.GetOrdinal("MQC2Id"));
                            ...
                            if (Enum.TryParse<MQC2ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = MQC2ProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(MQC2ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(MQC2ViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@MQC2Id", model.MQC2Id);
        ...
        public async Task<IActionResult> Delete(int MQC2Id)
        {
            ...
            string commandText = "DELETE FROM MQC2 WHERE MQC2Id = @MQC2Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@MQC2Id", MQC2Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted MQC2 with ID {MQC2Id}.");
