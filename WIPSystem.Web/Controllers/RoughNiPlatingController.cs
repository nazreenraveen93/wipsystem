...
{
    public class RoughNiPlatingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<RoughNiPlatingController> _logger;

        public RoughNiPlatingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<RoughNiPlatingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Rough Ni Plating' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT RoughNiPlatingId, PartNo, LotNo, OutputQty, Status 
            FROM RoughNiPlating 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<RoughNiPlatingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new RoughNiPlatingViewModel
                        {
                            RoughNiPlatingId = reader.GetInt32(reader.GetOrdinal("RoughNiPlatingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             RoughNiPlatingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<RoughNiPlatingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new RoughNiPlatingViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "I-Ki Etching"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(RoughNiPlatingViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the RoughNiPlating table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a RoughNiPlating record.");
        ...
        private void PrepareViewModel(RoughNiPlatingViewModel model)
        {
        ...
        public async Task<IActionResult> RoughNiPlatingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            RoughNiPlatingViewModel viewModel = null;
            ...
            string commandText = @"
                SELECT 
                    ...
                    p.PackageSize,
                    p.PiecesPerBlank
                FROM RoughNiPlating r
                INNER JOIN Machines m ON r.MachineId = m.MachineId
                INNER JOIN Products p ON r.PartNo = p.PartNo
                WHERE r.RoughNiPlatingId = @RoughNiPlatingId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@RoughNiPlatingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new RoughNiPlatingViewModel
                        {
                            RoughNiPlatingId = reader.GetInt32(reader.GetOrdinal("RoughNiPlatingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<RoughNiPlatingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_RoughNiPlatingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            RoughNiPlatingViewModel viewModel = new RoughNiPlatingViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.RoughNiPlatingId = reader.GetInt32(reader.GetOrdinal("RoughNiPlatingId"));
                            ...
                            if (Enum.TryParse<RoughNiPlatingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = RoughNiPlatingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(RoughNiPlatingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(RoughNiPlatingViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            string SDistUpdateCommandText = @"
        UPDATE RoughNiPlating
        SET OutputQty = @OutputQty, RejectQty = @RejectQty,
            Remarks = @Remarks, Status = @Status
        WHERE RoughNiPlatingId = @RoughNiPlatingId";
        ...
        WHERE PartNo = @PartNo AND LotNo = @LotNo";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sdistCommand = new SqlCommand(SDistUpdateCommandText, connection, transaction))
                    {
                        // Bind parameters for Sinter update
                        sdistCommand.Parameters.AddWithValue("@RoughNiPlatingId", model.RoughNiPlatingId);
                        sdistCommand.Parameters.AddWithValue("@OutputQty", model.OutputQty);
                        sdistCommand.Parameters.AddWithValue("@RejectQty", model.RejectQty);
                        sdistCommand.Parameters.AddWithValue("@Remarks", model.Remarks ?? (object)DBNull.Value); // Handle potential NULL value
                        sdistCommand.Parameters.AddWithValue("@Status", model.Status.ToString()); // Convert enum to string

                        var sinterResult = await sdistCommand.ExecuteNonQueryAsync();
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "RoughNiPlating process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the RoughNiPlating table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int RoughNiPlatingId)
        {
            ...
            string commandText = "DELETE FROM RoughNiPlating WHERE RoughNiPlatingId = @RoughNiPlatingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@RoughNiPlatingId", RoughNiPlatingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted RoughNiPlating with ID {RoughNiPlatingId}.");
