...
{
    public class CamberInspAllController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<CamberInspAllController> _logger;

        public CamberInspAllController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<CamberInspAllController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Camber Insp.All' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT CamberInspAllId, PartNo, LotNo, OutputQty, Status 
            FROM CamberInspAll 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<CamberInspAllViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new CamberInspAllViewModel
                        {
                            CamberInspAllId = reader.GetInt32(reader.GetOrdinal("CamberInspAllId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             CamberInspAllProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<CamberInspAllProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new CamberInspAllViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Camber Insp.All"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(CamberInspAllViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the CamberInspAll table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a CamberInspAll record.");
        ...
        private void PrepareViewModel(CamberInspAllViewModel model)
        {
        ...
        public async Task<IActionResult> CamberInspAllDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            CamberInspAllViewModel viewModel = null;
            ...
            string commandText = @"
                SELECT 
                    ...
                    p.PackageSize,
                    p.PiecesPerBlank
                FROM CamberInspAll c
                INNER JOIN Machines m ON c.MachineId = m.MachineId
                INNER JOIN Products p ON c.PartNo = p.PartNo
                WHERE c.CamberInspAllId = @CamberInspAllId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@CamberInspAllId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new CamberInspAllViewModel
                        {
                            CamberInspAllId = reader.GetInt32(reader.GetOrdinal("CamberInspAllId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<CamberInspAllProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_CamberInspAllDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            CamberInspAllViewModel viewModel = new CamberInspAllViewModel();
            ...
            INNER JOIN Machines m ON c.MachineId = m.MachineId
            INNER JOIN Products p ON c.PartNo = p.PartNo 
            LEFT JOIN CurrentStatus cs ON c.PartNo = cs.PartNo AND c.LotNo = cs.LotNo
            WHERE c.CamberInspAllId = @CamberInspAllId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.CamberInspAllId = reader.GetInt32(reader.GetOrdinal("CamberInspAllId"));
                            ...
                            if (Enum.TryParse<CamberInspAllProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = CamberInspAllProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(CamberInspAllViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(CamberInspAllViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@CamberInspAllId", model.CamberInspAllId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "CamberInspAll process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the CamberInspAll table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int CamberInspAllId)
        {
            ...
            string commandText = "DELETE FROM CamberInspAll WHERE CamberInspAllId = @CamberInspAllId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@CamberInspAllId", CamberInspAllId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted CamberInspAllId with ID {CamberInspAllId}.");
