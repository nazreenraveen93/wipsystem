...
{
    public class Sinter1Controller : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<Sinter1Controller> _logger;

        public Sinter1Controller(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<Sinter1Controller> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = '750Sinter1' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT Sinter1Id, PartNo, LotNo, OutputQty, Status 
            FROM Sinter1
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<Sinter1ViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new Sinter1ViewModel
                        {
                            Sinter1Id = reader.GetInt32(reader.GetOrdinal("Sinter1Id")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             Sinter1ProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<Sinter1ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new Sinter1ViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "750Sinter1"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Sinter1ViewModel model)
        {
        ...
        private void PrepareViewModel(Sinter1ViewModel model)
        {
        ...
        public async Task<IActionResult> Sinter1Details(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            Sinter1ViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE s.Sinter1Id = @Sinter1Id";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@Sinter1Id", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new Sinter1ViewModel
                        {
                            Sinter1Id = reader.GetInt32(reader.GetOrdinal("Sinter1Id")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<Sinter1ProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_Sinter1DetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            Sinter1ViewModel viewModel = new Sinter1ViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.Sinter1Id = reader.GetInt32(reader.GetOrdinal("Sinter1Id"));
                            ...
                            if (Enum.TryParse<Sinter1ProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = Sinter1ProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(Sinter1ViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(Sinter1ViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE PartNo = @PartNo AND LotNo = @LotNo";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(sinterUpdateCommandText, connection, transaction))
                    {
                        // Bind parameters for Sinter update
                        sinterCommand.Parameters.AddWithValue("@Sinter1Id", model.Sinter1Id);
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the Sinter table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int Sinter1Id)
        {
            ...
            string commandText = "DELETE FROM Sinter1 WHERE Sinter1Id = @Sinter1Id";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@Sinter1Id", Sinter1Id);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted Sinter1 with ID {Sinter1Id}.");
