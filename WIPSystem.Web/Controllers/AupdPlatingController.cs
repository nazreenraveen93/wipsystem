...
{
    public class AupdPlatingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<AupdPlatingController> _logger;

        public AupdPlatingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<AupdPlatingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Aupd Plating' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT AupdPlatingId, PartNo, LotNo, OutputQty, Status 
            FROM AupdPlating 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<AupdPlatingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new AupdPlatingViewModel
                        {
                            AupdPlatingId = reader.GetInt32(reader.GetOrdinal("AupdPlatingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             AupdPlatingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<AupdPlatingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new AupdPlatingViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Aupd Plating"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(AupdPlatingViewModel model)
        {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        (@PartNo, @LotNo, @ProcessCurrentStatus, @CheckedBy, @Status, @Remarks, @MachineId, @OutputQty, @RejectQty, @MachineStartTime, @MachineEndTime, @Date)";
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the AuPlating2 table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a AuPlating2 record.");
        ...
        private void PrepareViewModel(AupdPlatingViewModel model)
        {
        ...
        public async Task<IActionResult> AupdPlatingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            AupdPlatingViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE a.AupdPlatingId = @AupdPlatingId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@AupdPlatingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new AupdPlatingViewModel
                        {
                            AupdPlatingId = reader.GetInt32(reader.GetOrdinal("AupdPlatingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<AupdPlatingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_AupdPlatingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            AupdPlatingViewModel viewModel = new AupdPlatingViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.AupdPlatingId = reader.GetInt32(reader.GetOrdinal("AupdPlatingId"));
                            ...
                            if (Enum.TryParse<AupdPlatingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = AupdPlatingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(AupdPlatingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(AupdPlatingViewModel model)
        {
            if (!ModelState.IsValid)
            {
        ...
        WHERE AupdPlatingId = @AupdPlatingId";
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@AupdPlatingId", model.AupdPlatingId);
        ...
        public async Task<IActionResult> Delete(int AupdPlatingId)
        {
            ...
            string commandText = "DELETE FROM AupdPlating WHERE AupdPlatingId = @AupdPlatingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@AupdPlatingId", AupdPlatingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted AupdPlating with ID {AupdPlatingId}.");
