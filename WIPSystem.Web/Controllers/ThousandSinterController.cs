...
{
    public class ThousandSinterController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<ThousandSinterController> _logger;

        public ThousandSinterController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<ThousandSinterController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'ThousandSinter' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT SinterId, PartNo, LotNo, OutputQty, Status 
            FROM Sinter 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<SinterViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new SinterViewModel
                        {
                            SinterId = reader.GetInt32(reader.GetOrdinal("SinterId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             SinterProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<SinterProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new SinterViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "ThousandSinter"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(SinterViewModel model)
        {
        ...
        private void PrepareViewModel(SinterViewModel model)
        {
        ...
        public async Task<IActionResult> SinterDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            SinterViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE s.SinterId = @SinterId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@SinterId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new SinterViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<SinterProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_SinterDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            SinterViewModel viewModel = new SinterViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.SinterId = reader.GetInt32(reader.GetOrdinal("SinterId"));
                            ...
                            if (Enum.TryParse<SinterProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = SinterProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(SinterViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(SinterViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(sinterUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@SinterId", model.SinterId);
        ...
        public async Task<IActionResult> Delete(int SinterId)
        {
            ...
            string commandText = "DELETE FROM Sinter WHERE SinterId = @SinterId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@SinterId", SinterId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted Sinter with ID {SinterId}.");
