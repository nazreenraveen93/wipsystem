...
{
    public class NiPlatingController : Controller
    {
        private readonly WIPDbContext _wIPDbContext;
        private readonly IConfiguration _configuration;
        private readonly ILogger<NiPlatingController> _logger;

        public NiPlatingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<NiPlatingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Ni Plating' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT NiPlatingId, PartNo, LotNo, OutputQty, Status 
            FROM NiPlating 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<NiPlatingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var item = new NiPlatingViewModel
                        {
                            NiPlatingId = reader.GetInt32(reader.GetOrdinal("NiPlatingId")),
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             NiPlatingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<NiPlatingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
        ...
        {
            var viewModel = new NiPlatingViewModel
            {
                CurrentStatusId = CurrentStatusId,
                ProcessCurrentStatus = "Ni Plating"
            };
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
        ...
        [HttpPost]
        public async Task<IActionResult> Create(NiPlatingViewModel model)
        {
        ...
        private void PrepareViewModel(NiPlatingViewModel model)
        {
        ...
        public async Task<IActionResult> NiPlatingDetails(int id)
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            NiPlatingViewModel viewModel = null;
            ...
            string commandText = @"
                ...
                WHERE n.NiPlatingId = @NiPlatingId";

            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var command = new SqlCommand(commandText, connection);
                command.Parameters.AddWithValue("@NiPlatingId", id);
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        viewModel = new NiPlatingViewModel
                        {
                            NiPlatingId = reader.GetInt32(reader.GetOrdinal("NiPlatingId")),
                            PartNo = reader.GetString(reader.GetOrdinal("PartNo")),
                            LotNo = reader.GetString(reader.GetOrdinal("LotNo")),
                            ...
                            Status = EnumExtensions.ConvertToEnum<NiPlatingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_NiPlatingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            NiPlatingViewModel viewModel = new NiPlatingViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.NiPlatingId = reader.GetInt32(reader.GetOrdinal("NiPlatingId"));
                            ...
                            if (Enum.TryParse<NiPlatingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = NiPlatingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(NiPlatingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(NiPlatingViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var niplatingCommand = new SqlCommand(niplatingUpdateCommandText, connection, transaction))
                    {
                        ...
                        niplatingCommand.Parameters.AddWithValue("@NiPlatingId", model.NiPlatingId);
        ...
        public async Task<IActionResult> Delete(int NiPlatingId)
        {
            ...
            string commandText = "DELETE FROM Ni Plating WHERE NiPlatingId = @NiPlatingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    command.Parameters.AddWithValue("@NiPlatingId", NiPlatingId);

                    int result = await command.ExecuteNonQueryAsync();
                    if (result > 0)
                    {
                        // Record was successfully deleted
                        _logger.LogInformation($"Deleted NiPlating with ID {NiPlatingId}.");
