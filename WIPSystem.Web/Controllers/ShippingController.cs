...
namespace WIPSystem.Web.Controllers
{
    public class ShippingController : Controller
    {
        ...
        public ShippingController(WIPDbContext wIPDbContext, IConfiguration configuration, ILogger<ShippingController> logger)
        {
        ...
        public IActionResult Index()
        {
            ...
            var query = "SELECT CurrentStatusId, PartNo, LotNo, ReceivedQuantity,Date, Status FROM CurrentStatus WHERE ProcessCurrentStatus = 'Shipping' AND Status = 'In Progress'";
        ...
        public IActionResult Index1()
        {
            ...
            // Adjusted query to include both conditions
            var query = @"
            SELECT ShippingId, PartNo, LotNo, OutputQty, Status 
            FROM Shipping 
            WHERE Status IN ('Completed', 'On Hold')";

            var model = new List<ShippingViewModel>(); // Assuming CurrentStatus is your model class
            using (var connection = new SqlConnection(connectionString))
            {
                var command = new SqlCommand(query, connection);
                connection.Open();
                    ...
                    while (reader.Read())
                    {
                        var item = new ShippingViewModel
                        {
                            ...
                            Status = reader.IsDBNull(reader.GetOrdinal("Status")) ?
                             ShippingProcessStatus.Completed : // or some default status
                             EnumExtensions.ConvertToEnum<ShippingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (var insertCommand = new SqlCommand(insertCommandText, connection, transaction))
                            {
                                ...
                                {
                                    ModelState.AddModelError("", "No records were inserted into the Shipping table.");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError(ex, "An error occurred while creating a Shipping record.");
        ...
        private void PrepareViewModel(ShippingViewModel model)
        {
        ...
        {
            var connectionString = _configuration.GetConnectionString("DefaultConnection");
            ShippingViewModel viewModel = null;
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        viewModel = new ShippingViewModel
                        {
                            ...
                            Status = EnumExtensions.ConvertToEnum<ShippingProcessStatus>(reader.GetString(reader.GetOrdinal("Status"))),
            ...
            return PartialView("_ShippingDetailsModal", viewModel);
        }

        [HttpGet]
        public async Task<IActionResult> Edit(int id)
        {
            ShippingViewModel viewModel = new ShippingViewModel();
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        if (await reader.ReadAsync())
                        {
                            viewModel.ShippingId = reader.GetInt32(reader.GetOrdinal("ShippingId"));
                            ...
                            if (Enum.TryParse<ShippingProcessStatus>(statusString, true, out var parsedStatus))
                            {
                            ...
                            else
                            {
                                ...
                                viewModel.Status = ShippingProcessStatus.OnHold; // Or another appropriate default value
                            }
        ...
        private async Task PrepareViewModelForEdit(ShippingViewModel model)
        {
            // Retrieve the process ID based on the current status name.
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(ShippingViewModel model)
        {
            if (!ModelState.IsValid)
            {
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    using (var sinterCommand = new SqlCommand(burrUpdateCommandText, connection, transaction))
                    {
                        ...
                        sinterCommand.Parameters.AddWithValue("@ShippingId", model.ShippingId);
                        ...
                        if (sinterResult > 0)
                        {
                            using (var currentStatusCommand = new SqlCommand(currentStatusUpdateCommandText, connection, transaction))
                            {
                                ...
                                {
                                    transaction.Commit();
                                    TempData["Success"] = "Shipping process and current status updated successfully.";
                                    return RedirectToAction("Index1");
                                }
                                else
                                {
                        ...
                        {
                            transaction.Rollback();
                            ModelState.AddModelError("", "No records were updated in the Shipping table.");
                            await PrepareViewModelForEdit(model);
                            return View(model);
                        }
                    }
        ...
        public async Task<IActionResult> Delete(int ShippingId)
        {
            ...
            string commandText = "DELETE FROM Shipping WHERE ShippingId = @ShippingId";
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var command = new SqlCommand(commandText, connection))
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        _logger.LogInformation($"Deleted ShippingId with ID {ShippingId}.");
